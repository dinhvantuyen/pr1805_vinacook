class ProductsController < ApplicationController
  ...
  def index
    if params[:search_name].present? && params[:category_id].present?
      @products = Product.search_product_name params[:search_name], params[:category_id]
      flash[:info] = "Không có sản phẩm nào được tìm thấy!" if @products.empty?
    elsif params[:search_name].present?
      @products = Product.search_name params[:search_name]
      flash[:info] = "Không có sản phẩm nào được tìm thấy!" if @products.empty?
    elsif params[:category_id].present?
      @products = Product.search_product_category params[:category_id]
      flash[:info] = "Không có sản phẩm nào được tìm thấy!" if @products.empty?
    else
